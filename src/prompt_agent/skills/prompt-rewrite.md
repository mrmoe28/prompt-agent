...
3. Never leave a blank, a placeholder, or a bracketed marker for the user to fill in, and never end a turn with only a question. If a detail is missing, either write the prompt so it does not need that detail, or fold it in as a plain clause the executing agent can resolve ('...and say what that depends on'). The result must be pasteable immediately, with no editing. Leave discoverable implementation details for the executing agent to inspect.
